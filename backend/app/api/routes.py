...
async def generate_posts(
    ...
    try:
        ...
        context_posts = {}
        if request.x_handle:
            logger.info(f"Retrieving context posts for x_handle @{request.x_handle}")
            for platform in request.target_platforms:
                # Map platform names for database lookup (twitter -> x)
                db_platform = "x" if platform == "twitter" else platform
                platform_context = await pipeline_service.get_context_posts_by_handle(request.x_handle, db_platform)
                ...
                logger.info(f"Found {len(context_posts[platform])} context posts for platform {platform} (db: {db_platform})")
        elif request.user_id:
            logger.info(f"Retrieving context posts for user_id {request.user_id}")
            for platform in request.target_platforms:
                platform_context = await pipeline_service.get_user_context_posts(request.user_id, platform)
                context_posts[platform] = sorted([post["post_content"] for post in platform_context], key=len, reverse=True)[:10] # Sort by length and take the longest 10
                logger.info(f"Found {len(context_posts[platform])} context posts for platform {platform}")
...
async def stream_posts(
    ...
    try:
        ...
        # Get context posts from database if x_handle is provided
        context_posts = {}
        if request.x_handle:
            logger.info(f"Retrieving context posts for x_handle @{request.x_handle}")
            for platform in request.target_platforms:
                # Map platform names for database lookup (twitter -> x)
                db_platform = "x" if platform == "twitter" else platform
                platform_context = await streaming_pipeline_service.get_context_posts_by_handle(request.x_handle, db_platform)
                context_posts[platform] = sorted([post["post_content"] for post in platform_context], key=len, reverse=True)[:10] # Sort by length and take the longest 10
                logger.info(f"Found {len(context_posts[platform])} context posts for platform {platform} (db: {db_platform})")
        elif request.user_id:
            logger.info(f"Retrieving context posts for user_id {request.user_id}")
            for platform in request.target_platforms:
                platform_context = await streaming_pipeline_service.get_user_context_posts(request.user_id, platform)
                context_posts[platform] = sorted([post["post_content"] for post in platform_context], key=len, reverse=True)[:10] # Sort by length and take the longest 10
                logger.info(f"Found {len(context_posts[platform])} context posts for platform {platform}")
        else:
            logger.info("No x_handle or user_id provided, skipping context post retrieval")
