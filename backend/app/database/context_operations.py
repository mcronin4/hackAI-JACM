...
class ContextPostsDB:
    ...
    async def user_has_context(self, user_id: str) -> bool:
        """Check if user already has context posts in database"""
        try:
            response = self.client.table(self.table_name).select("id").eq("user_id", user_id).limit(1).execute()
            return len(response.data) > 0
        except Exception as e:
            logger.error(f"Error checking user context: {str(e)}")
            return False
    
    async def save_context_posts(self, user_id: str, posts: List[Dict[str, Any]], platform: str = "x") -> bool:
